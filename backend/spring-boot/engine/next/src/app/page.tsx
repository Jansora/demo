...
import {Form, FormControl, FormField, FormItem, FormLabel, FormMessage,} from "@jansora/ui/esm/components/ui/form"
...
export default function Page() {
    ...
    async function onSubmit(values: any) {
        ...
        if (result.data.status) {
            toast.success('试算成功')
            setCalculation(result.data.data)
                ...
                <div className="w-2/3 mx-auto">
                    <Tabs defaultValue="calculate" className="w-full">
                        <TabsList className="grid w-full grid-cols-2">
                            <TabsTrigger value="template">元数据编辑</TabsTrigger>
                        ...
                        <TabsContent value="calculate" className="pt-8">
                            <Accordion type="multiple" className="w-full px-5" defaultValue={["item-1","item-2","item-3","item-4"]}>
                                ...
                                <AccordionItem value="item-2">
                                    <AccordionTrigger className="hover:no-underline">输入变量</AccordionTrigger>
                                    <AccordionContent>
                                ...
                                <AccordionItem value="item-3">
                                    <AccordionTrigger className="hover:no-underline">工资计算块</AccordionTrigger>
                                    <AccordionContent>
                                                    ...
                                                    return <Card className="w-[450px] mt-2 mr-2" key={index}>
                                                        ...
                                                        <CardContent>
                                                            ...
                                                            <span className="text-sm text-orange-500">
                                                                {!!calculation[formula] && !!calculation[formula][item] && calculation[formula][item]}
                                                            </span>
                                                            </span>
                                                                    })
                                                                }
                                 ...
                                 <span>
                                                ...
                                                <span className="text-sm text-green-500">
                                                  ...
                                                  <span className="text-sm text-orange-500">
                                                    {calculation[formula] && Object.values(calculation[formula]).reduce((sum: number, value: number) => sum + value, 0)}
                                                </span>
                                                </span>
                                ...
                                <AccordionItem value="item-4">
                                    <AccordionTrigger className="hover:no-underline">计算结果</AccordionTrigger>
                                    <AccordionContent>
                                        <p className="mt-3 text-lg">最终工资计算
                                            <code className="text-cyan-500"> {metadata.finalFormula}</code>
                                            <span className="text-sm text-orange-500">
                                                    {calculation['结果'] ? calculation['结果'] : 0 }
                                                </span>
                                        </p>
                                    </AccordionContent>
                                </AccordionItem>
